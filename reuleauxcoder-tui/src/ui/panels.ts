import type {TuiController} from '../state/controller.js';
import {humanize} from '../state/menus.js';
import {diff, fields, safe, wrap} from './format.js';
import {paint} from './theme.js';
import {inputRows, line, selectionRows} from './viewport.js';

export interface PanelRows {title: string; rows: string[]; hint: string}
export function panelRows(c: TuiController, width: number, height: number): PanelRows | null {
  if (c.active) {
    const {kind, request, expiresAt} = c.active;
    const waiting = Math.max(c.client.interactions.length - 1, c.session.state.approval_waiting);
    const title = `${request.title}${waiting ? ` · ${waiting} waiting` : ''}${expiresAt === null ? '' : ` · ${Math.max(0, Math.ceil((expiresAt - performance.now()) / 1000))}s`}`;
    if (kind === 'input_text' || c.interactionMode === 'feedback') {
      const prompt = c.interactionMode === 'feedback' ? 'Explain what should change before this runs:' : request.prompt;
      const intro = wrap(safe(prompt), width);
      const introHeight = Math.max(0, Math.min(intro.length, height - 2));
      const prefix = introHeight ? [...intro.slice(0, introHeight), ''] : [];
      return {title, rows: [...prefix, ...inputRows(c.interactionInput, width, Math.max(1, height - prefix.length), true, request.secret)], hint: `${request.secret ? 'Masked · ' : ''}Enter submit · Alt+Enter newline · Esc cancel`};
    }
    if (kind === 'choose_one' || c.interactionMode === 'scope') {
      const items = kind === 'choose_one' ? request.items : request.grant_options.map((item: any) => ({...item, label: item.label + (item.broad ? ' (broad scope)' : '')}));
      const intro = kind === 'choose_one' ? request.message : 'Allow matching requests for this session:';
      return {title, rows: [...(intro ? [line(safe(intro), width)] : []), ...selectionRows(items, c.interactionIndex, width, height - (intro ? 1 : 0))], hint: '↑↓ select · Enter confirm · Esc back'};
    }
    const context = request.context;
    const contextText = context ? [
      [context.tool_name, context.tool_source, context.operation].filter(Boolean).join(' · '),
      context.subjects.join('\n'), context.reason,
      context.is_subagent ? `Subagent${context.subagent_mode ? ' · ' + context.subagent_mode : ''}${context.subagent_task ? '\n' + context.subagent_task : ''}` : '',
    ].filter(Boolean).map(safe).join('\n') : '';
    const sections = kind === 'confirm' ? safe(request.message) : [safe(request.summary), contextText, ...request.sections.map((section: any) => `${paint.bold(safe(section.title ?? section.kind))}\n${section.kind === 'diff' ? diff(section.content) : fields(section.content)}`)].filter(Boolean).join('\n\n');
    const rows = wrap(sections, width);
    c.interactionOffset = Math.min(c.interactionOffset, Math.max(0, rows.length - height));
    return {title, rows: rows.slice(c.interactionOffset, c.interactionOffset + height), hint: kind === 'confirm' ? 'y/Enter yes · n no · Esc cancel' : `y/Enter ${request.approve_label} once · n ${request.reject_label}${request.grant_options.length ? ' · s scope' : ''} · f feedback · PgUp/PgDn scroll (${c.interactionOffset + 1}/${rows.length})`};
  }
  const screen = c.screen;
  if (screen?.kind === 'list') {
    const items = c.listItems(screen);
    screen.index = Math.min(screen.index, Math.max(0, items.length - 1));
    const filter = height > 1 ? inputRows(screen.filter, width - 2, 1).map(row => '⌕ ' + row + (screen.filter.text ? '' : paint.muted('Filter options…'))) : [];
    return {title: screen.title, rows: [...filter, ...selectionRows(items, screen.index, width, height - filter.length)], hint: `↑↓ select · Enter open · Esc back · ${items.length ? screen.index + 1 : 0}/${items.length}`};
  }
  if (screen?.kind === 'document') {
    const rows = wrap(safe(screen.body), width);
    screen.offset = Math.min(screen.offset, Math.max(0, rows.length - height));
    return {title: screen.title, rows: rows.slice(screen.offset, screen.offset + height), hint: `↑↓ / PgUp PgDn scroll · Esc back${screen.menu ? ' · Tab actions' : ''} · ${screen.offset + 1}/${rows.length}`};
  }
  if (screen?.kind === 'form') {
    const parameter = screen.action.parameters[screen.index];
    return {title: screen.title, rows: [line(`${humanize(parameter.name)} · ${screen.index + 1}/${screen.action.parameters.length}${parameter.required ? ' · required' : ''}`, width), ...wrap(parameter.kind === 'boolean' ? 'Tab changes value: true / false' + (parameter.nullable ? ' / auto' : '') : parameter.kind === 'integer' ? 'Enter a whole number.' : 'Enter text.', width), '', ...inputRows(screen.input, width, Math.max(1, height - 4)), ...(screen.error ? [line(paint.error(screen.error), width)] : [])].slice(0, height), hint: 'Enter next / submit · ↑ previous field · Esc back'};
  }
  if (c.palette.length) return {title: 'Commands', rows: selectionRows(c.palette.map(menu => ({label: menu.name, description: menu.title})), c.paletteIndex % c.palette.length, width, height), hint: '↑↓ select · Tab complete · Enter open · Esc dismiss'};
  return null;
}
