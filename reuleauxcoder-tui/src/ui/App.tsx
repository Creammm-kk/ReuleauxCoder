...
import {inputRows, line, statusLine, TranscriptLayout} from './viewport.js';
import {panelRows} from './panels.js';
import {useAlternateScroll, useTerminalKeys} from './terminal.js';
import {between, fit, section, rail, keyHint, paint} from './theme.js';
...
function Rows({rows, height, width}: {rows: string[]; height: number; width: number}) {
  return <Box flexDirection="column" height={height} flexShrink={0}>{Array.from({length: height}, (_, index) => <Text key={index} wrap="truncate">{line(rows[index] || ' ', width)}</Text>)}</Box>;
}
...
export function App({controller: c, alternateScreen = false}: {controller: TuiController; alternateScreen?: boolean}) {
  ...
  const width = Math.max(8, c.columns - 2);
  const height = Math.max(6, c.rows - 1);
  const headerHeight = height >= 18 ? 3 : 2;
  const liveActivity = activityFor(c);
  const composerWidth = Math.max(1, width - 4);
  ...
  const panelCapacity = hasPanel ? Math.max(1, Math.min(desiredPanelHeight, available - (c.active ? 2 : 5))) : 0;
  ...
  const phaseColor = c.session.fatal ? paint.error : c.active || state.stopping ? paint.warning : state.running || !c.session.connected ? paint.accent : paint.success;
  const header = between(paint.bold(' ◭ REULEAUX / CODER'), '● ' + phase + ' ', width);
  const modelLine = statusLine([state.model, context, state.mcp_tools ? `MCP ${state.mcp_tools} tools` : '', state.workspace], width - 2);
  ...
  const shortcutHints = (panel
    ...
    : [keyHint('F4', c.expanded ? 'collapse output + reasoning' : 'show tool output + reasoning'), keyHint('F2', 'session details'), keyHint('/', 'commands'), keyHint('PgUp', 'history')]
  ).join(paint.muted('  ·  '));
  ...
  const composer = inputRows(c.composer, composerWidth, composerHeight, focused).map((row, index) => rail(
    (index ? '  ' : inputColor('▸ ')) + row + (!c.composer.text && !index && focused ? paint.muted('Describe your next change…') : ''), width, inputColor,
  ));
  const composerHint = focused ? width < 55 ? 'Enter send · Alt+Enter newline' : state.running ? 'Enter queue message · Ctrl+C interrupt' : 'Enter send · Alt+Enter newline · Alt+↑↓ history' : 'Draft preserved';
  const welcome = [
  ...
  return <Box flexDirection="column" paddingLeft={1} paddingRight={1} width={c.columns} height={height} overflow="hidden">
    <Rows rows={[paint.selected(fit(header, width)), phaseColor('▏') + ' ' + paint.muted(modelLine), section(sessionLabel, [c.expanded ? 'Full details · F4 collapse' : '', activity].filter(Boolean).join(' · '), width)]} height={headerHeight} width={width}/>
    ...
    {panel && <>
      <Rows rows={[section(panelKind, safe(panel.title), width, panelColor)]} height={1} width={width}/>
      <Rows rows={Array.from({length: panelHeight}, (_, index) => rail(panel.rows[index] || '', width, panelColor))} height={panelHeight} width={width}/>
      <Rows rows={[rail(paint.muted(panel.hint), width, panelColor)]} height={1} width={width}/>
    </>}
    {queue.length > 0 && <Rows rows={queue} height={queue.length} width={width}/>}
    <Rows rows={[section(focused ? state.running ? 'YOU / STEERING' : 'YOU' : 'DRAFT', '', width, inputColor)]} height={1} width={width}/>
    <Rows rows={composer} height={composerHeight} width={width}/>
    <Rows rows={[rail(paint.muted(composerHint), width, inputColor)]} height={1} width={width}/>
    <Rows rows={[footer]} height={1} width={width}/>
  </Box>;
}
