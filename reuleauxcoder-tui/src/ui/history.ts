...
export function historyRows(browser: HistoryBrowser, width: number, height: number, layout = new TextLayout()): PanelRows {
  ...
  if (browser.search) header.push(...inputRows(browser.search, width - 2, 1).map(row => '⌕ ' + row));
  let rows: string[];
  if (browser.detailed) {
  ...
  return {title, rows: [...header, ...rows].slice(0, height), hint, navigation: `Page ${browser.cursors.length}${browser.nextCursor ? ' · more' : ''}`};
}
