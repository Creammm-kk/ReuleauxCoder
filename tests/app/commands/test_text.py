"""Unknown and malformed slash input stays inside the command application."""
...
from reuleauxcoder.app.commands.text import (
    _extract_base_name,
...
def test_invalid_known_command_reports_registered_usage() -> None:
    ...
    message = _invalid_command_usage("/thinking effort xml", registry, _CLI_PROFILE)
...
def test_invalid_known_slash_command_never_falls_through_to_chat() -> None:
    ...
    registry = ActionRegistry(
        [
            ActionSpec(
                ...
                triggers=(
                    TriggerSpec(
                        kind=TriggerKind.SLASH, value="/thinking effort <level>"
                    ),
                ),
    ...
    ui_bus = SimpleNamespace(warning=lambda message, **kwargs: warnings.append(message))

    result = CommandService(
        agent=SimpleNamespace(current_session_id="s1"),
        config=SimpleNamespace(),
        ui_bus=ui_bus,
        ui_profile=_CLI_PROFILE,
        registry=registry,
    ).submit("/thinking effort invalid")

    assert result.control == "continue"
    assert warnings == ["Invalid '/thinking' command. Usage: /thinking effort <level>"]
